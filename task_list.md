...
### [x] Task 11. T6 테스트 재실행 — 프로젝트 더블클릭 + 세금계산서 1건 검증 ✅ (세션 XIV 완료)
- 상세 내용:
...
### [x] Task 13. 예산과목 필드 재조사 및 수정 ✅ (세션 XIV 완료)
- 상세 내용:
...
### [x] Task 19. 예산과목 실 GW 테스트 및 검증 ✅ (세션 XIV 완료)
- **완료 내용 (세션 XIV)**:
  - `budget_helpers._click_budget_field()`: y=990 필드 정상 탐지 (viewport height 60% threshold)
  - `_click_sub_popup_confirm()`: 350px 거리 제한 추가 → 서브팝업 내부 버튼만 타겟팅
  - `_confirm_budget_modal()`: 서브팝업 닫힘 대기 + 메인모달 닫힘 확인 로직 추가
  - 지급요청일 그리드 컬럼 header dict 파싱 버그 수정 (dict.replace() → str 변환)
  - T6 PASS (240.1s), 7/13 PASS 달성
