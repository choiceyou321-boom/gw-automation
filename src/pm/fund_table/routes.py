...
async def create_project(req: ProjectCreate, request: Request):
    ...
    if project_code and new_id:
        ...
        def _auto_crawl():
            try:
                from src.pm.fund_table.project_crawler import crawl_project_info
            ...
            # 사업별 크롤러 (메인 — 전기+당기)
            try:
                from src.pm.fund_table.budget_crawler_by_project import crawl_budget_by_project
            ...
            try:
                from src.pm.fund_table.budget_crawler import crawl_budget_summary
...
async def crawl_gw_all(request: Request):
    ...
    각 단계 독립 try/except (한쪽 실패해도 계속)
    """
    from src.pm.fund_table.scheduler import sync_running
    ...
    def _crawl():
        ...
        # 1단계: 프로젝트 등록정보 크롤링
        try:
            from src.pm.fund_table.project_crawler import crawl_all_project_info
        ...
        try:
            from src.pm.fund_table.budget_crawler_by_project import crawl_all_by_project
        ...
        try:
            from src.pm.fund_table.budget_crawler import crawl_all_summary
...
async def fetch_gw_project_list(request: Request):
    ...
    def _fetch():
        try:
            from src.pm.fund_table.project_crawler import search_gw_projects as _search_fn
...
async def import_pm_sheet(request: Request):
    ...
    def _import():
        try:
            from src.pm.fund_table.sheets_import import import_from_pm_sheet
...
async def sync_budget_actuals(project_id: int, request: Request):
    ...
    def _sync():
        try:
            from src.pm.fund_table.budget_crawler import crawl_budget_actual
...
async def sync_all_budget_actuals(request: Request):
    ...
    def _sync_all():
        try:
            from src.pm.fund_table.budget_crawler import crawl_all_projects
...
async def sync_tax_invoices_simple(project_id: int, request: Request):
    ...
    def _sync():
        try:
            from src.pm.fund_table.tax_invoice_crawler import TaxInvoiceCrawler
...
async def sync_collection_schedule(project_id: int, request: Request):
    ...
    def _sync():
        try:
            from src.pm.fund_table.collection_schedule_crawler import CollectionScheduleCrawler
            from playwright.sync_api import sync_playwright
            from src.shared.auth.login import login_and_get_context, close_session
            from src.shared.auth.user_db import get_decrypted_password
...
async def sync_payment_approvals(project_id: int, request: Request):
    ...
    def _sync():
        try:
            from src.pm.fund_table.payment_approval_crawler import PaymentApprovalCrawler
            from playwright.sync_api import sync_playwright
            from src.shared.auth.login import login_and_get_context, close_session
            from src.shared.auth.user_db import get_decrypted_password
...
async def import_schedule_pm_sheet(request: Request):
    ...
    try:
        import asyncio
        from src.pm.fund_table.sheets_import import import_schedule_from_pm_sheet
...
async def import_collections_pm_sheet(request: Request):
    ...
    try:
        import asyncio
        from src.pm.fund_table.sheets_import import import_collections_from_pm_sheet
...
async def get_process_map_trades(request: Request, type: str = "오피스"):
    ...
    if trades:
        ...
        # DB 비어있으면 하드코딩 폴백
        from src.pm.fund_table.process_map_master import PROCESS_GROUPS, get_preset_trades
...
async def parse_estimate(request: Request):
    ...
    try:
        import asyncio
        from src.pm.fund_table.estimate_parser import parse_estimate_file
...
async def generate_schedule(project_id: int, request: Request):
    ...
    try:
        ...
        from src.pm.fund_table.schedule_export import export_schedule_xlsx, export_schedule_pdf
