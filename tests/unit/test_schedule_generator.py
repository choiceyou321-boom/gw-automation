...
class TestAreaFactor:
    ...
    def test_base_100_returns_one(self):
        """100평 기준 → 1.0"""
        from src.fund_table.schedule_generator import _area_factor
...
class TestDAGValidation:
    ...
    def test_valid_dag_passes(self):
        ...
        from src.fund_table.schedule_generator import _validate_dag
        trades = [
            {"name": "A", "predecessors": []},
...
class TestProcessMapMaster:
    ...
    def test_get_trade_map(self):
        from src.fund_table.process_map_master import get_trade_map
        tm = get_trade_map()
    ...
    def test_type_presets_no_duplicate(self):
        """프리셋에 중복 공종 없어야 함"""
        from src.fund_table.process_map_master import TYPE_PRESETS
    ...
    def test_all_preset_trades_exist_in_master(self):
        ...
        from src.fund_table.process_map_master import TYPE_PRESETS, get_all_trade_names
    ...
    def test_predecessors_reference_valid_trades(self):
        ...
        from src.fund_table.process_map_master import PROCESS_GROUPS, get_all_trade_names
        all_names = set(get_all_trade_names())
        for grp in PROCESS_GROUPS:
            for item in grp["items"]:
...
class TestEstimateParser:
    ...
    def test_match_trade_alias(self):
        """별칭 매칭 테스트"""
        from src.fund_table.estimate_parser import _match_trade
        from src.fund_table.process_map_master import get_all_trade_names
