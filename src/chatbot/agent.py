...
AUTOMATION_TOOLS = [
    types.Tool(function_declarations=[
        ...
        types.FunctionDeclaration(
            ...
            description=(
                "프로젝트 자금현황 요약을 조회합니다. "
                "사용자가 '자금현황', '자금관리표', '프로젝트 예산', '수익금', '하도급 현황', "
...
def handle_get_fund_summary(params: dict, user_context: dict = None) -> str:
    ...
    if project_name:
        ...
        return (
            f"📊 **{summary['project_name']}** 자금현황 요약\n\n"
            f"• 수주액: {fmt(summary['total_order'])} (설계 {fmt(summary['design_amount'])} + 시공 {fmt(summary['construction_amount'])})\n"
            f"• 실행예산: {fmt(summary['execution_budget'])}\n"
            f"• 수익금: {fmt(summary['profit_amount'])} (이익율 {summary['profit_rate']:.1f}%)\n\n"
            f"• 공종 수: {summary['trade_count']}개\n"
            f"• 하도급 업체: {summary['total_companies']}개사\n"
            f"• 계약총액: {fmt(summary['total_contract'])}\n"
            f"• 기지급액: {fmt(summary['total_paid'])}\n"
            f"• 잔여금액: {fmt(summary['total_remaining'])}\n\n"
            f"상세 내용은 자금관리 페이지(/fund)에서 확인하세요."
        )
    else:
        # 전체 프로젝트 요약
        summaries = get_all_projects_summary()
        if not summaries:
            return "등록된 프로젝트가 없습니다. 자금관리 페이지(/fund)에서 프로젝트를 먼저 등록해주세요."
        ...
        lines.append(f"\n총 {len(summaries)}개 프로젝트. 상세는 자금관리 페이지(/fund)에서 확인하세요.")
        return "\n".join(lines)
