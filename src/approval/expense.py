...
class ExpenseReportMixin:
    ...
    def _fill_expense_fields(self, data: dict):
        ...
        from src.approval.expense_fields import fill_expense_fields
    ...
    def _fill_project_code(self, project: str, y_hint: float = None):
        """프로젝트 코드도움 모달 — `src.approval.project_picker`로 위임."""
    ...
    def _select_invoice_in_modal(
        ...
        """매입(세금)계산서 모달 — `src.approval.invoice_modal`로 위임."""
        from src.approval.invoice_modal import select_invoice_in_modal
    ...
    def _upload_attachment(self, file_path: str) -> bool:
        """첨부파일 업로드 — `src.approval.attachment.upload_attachment`로 위임."""
    ...
    def capture_budget_status_screenshot(self, output_path: str = None, detail_view: bool = True) -> str | None:
        """예실대비현황(상세) 스크린샷 — `src.approval.budget_capture`로 위임."""
        from src.approval.budget_capture import capture_budget_status_screenshot
        return capture_budget_status_screenshot(self.page, output_path, detail_view)

    def _click_budget_detail_view(self) -> bool:
        """예실대비현황 "상세" 뷰 클릭 — `src.approval.budget_capture`로 위임."""
        from src.approval.budget_capture import click_budget_detail_view
