"""
Phase 0 v2: 전자결재 양식 DOM 탐색 스크립트
- 전자결재 HOME → 추천양식 "[프로젝트]지출결의서" 직접 클릭
...
OUTPUT_DIR = PROJECT_ROOT / "data" / "approval_dom_v2"
...
def dump_table_structure(ctx, name):
    ...
    info = ctx.evaluate("""() => {
        ...
        document.querySelectorAll('table').forEach((table, ti) => {
            if (table.offsetParent === null) return;  // 보이는 테이블만
            const rows = [];
            ...
            if (rows.length > 0) {
                tables.push({ti: ti, id: table.id, className: table.className.substring(0, 100), rows: rows.slice(0, 40)});
            }
        });
        return tables;
    }""")
...
def main():
    print("=" * 60)
    print("Phase 0 v2: 전자결재 DOM 탐색 (추천양식 직접 클릭)")
    print("=" * 60)
    ...
    try:
        ...
        # 방법: 전자결재 모듈 아이콘 클릭 (span.module-link.EA)
        ...
        # 3. 추천양식에서 "[프로젝트]지출결의서" 클릭
        print("\n[3/7] 추천양식에서 지출결의서 클릭...")

        form_clicked = False
        for keyword in ["[프로젝트]지출결의서", "지출결의서", "프로젝트]지출"]:
            try:
        ...
        if form_page and form_page != page:
            ...
            for p in all_pages[1:]:
                try:
                    ...
                    pass
            page = all_pages[0]
            print(f"  팝업 닫고 원래 페이지 사용: {page.url[:100]}")
        ...
        action_info = page.evaluate("""() => {
            const result = [];
            const keywords = ['보관', '상신', '임시저장', '미리보기', '결재선', '취소', '닫기'];
