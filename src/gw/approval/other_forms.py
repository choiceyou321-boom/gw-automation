...
from src.gw.approval.base import GW_URL, MAX_RETRIES, RETRY_DELAY, SCREENSHOT_DIR, _GET_GRID_IFACE_JS, _save_debug, _js_str
...
class OtherFormsMixin:
    ...
    def _fill_advance_grid_mandatory_fields(self, data: dict):
        ...
        if usage_code and budget_keyword:
            try:
                from src.gw.approval.budget_helpers import handle_auto_triggered_popup
        ...
        if usage_code and budget_keyword and not _budget_handled:
            try:
                from src.gw.approval.budget_helpers import select_budget_code
                _proj_kw = project.split(". ", 1)[-1].split("]")[-1].strip() if project else ""
